...
module dcache #(
    ...
    typedef struct packed {
        logic dirty;
        logic [p.line_addr_width-1:0] tag;
        logic [p.line_width-1:0] data;
    } line_s;

    typedef union packed {
        logic [p.line_width/8-1:0][7:0] b8;
        logic [p.line_width/16-1:0][15:0] b16;
        logic [p.line_width/32-1:0][31:0] b32;
        logic [p.line_width/64-1:0][63:0] b64;
    } dcache_data_s;

    // Cache line size must be divisible by 64 bits.
    initial `assertEqual(0, p.line_width % 64);

    // The data of the cache lines.
    dcache_data_s datas [depth-1:0];

    // The tags of the cache lines.
    logic [p.line_addr_width-1:0] tags [depth-1:0];

    // The dirty flags of the cache lines.
    logic [depth-1:0] dirty_flags;

    // The address of the cache line being accessed.
    wire [p.line_addr_width-1:0] line_addr = bus.addr[
        p.addr_width - 1
        : p.addr_width - p.line_addr_width
    ];
    ...
    logic [p.line_addr_width-1:0] last_addr;
    ...
    localparam addr_width_8bit = $clog2(p.line_width / 8);
    localparam addr_width_16bit = $clog2(p.line_width / 16);
    localparam addr_width_32bit = $clog2(p.line_width / 32);
    localparam addr_width_64bit = $clog2(p.line_width / 64);
    ...
    always_ff @(posedge clk_i) begin
        if (bus.r_req || bus.w_req) begin
            ...
            if (p.line_addr_width'(tags[set]) != line_addr) begin
                line.data <= datas[set];
            end else casez (bus.r_size)
                DCACHE_DATA_8_BITS:
                    line.data <= p.line_width'(datas[set].b8[addr_8bit]);
                DCACHE_DATA_16_BITS:
                    line.data <= p.line_width'(datas[set].b16[addr_16bit]);
                DCACHE_DATA_32_BITS:
                    line.data <= p.line_width'(datas[set].b32[addr_32bit]);
                DCACHE_DATA_64_BITS:
                    line.data <= p.line_width'(datas[set].b64[addr_64bit]);
            endcase
        end
    end
